<template>
  <div class="l2_main">
    ...
    >
      <span class="l2_button" :style="{color: underline[0].color, fontWeight:underline[0].type}">
        사용자 수
      </span>
      <span class="l2_button" :style="{color: underline[1].color, fontWeight:underline[1].type}">
        국내 어린이집 수
      </span>
      <span class="l2_button" :style="{color: underline[2].color, fontWeight:underline[2].type}">
        리뷰 수
      </span>
    </div>
    <div align="center">
  ...
  methods: {
    ...
    test () {
      ...
      setInterval(function () {
        if (i % 3 === 0) {
          for (let j = 0; j < 3; j++) {
            ul[j] = { color: 'black', type: 'normal' }
          }
          ul[0] = { color: '#0CCB8A', type: '800' }
          uc(euc)
        } else if (i % 3 === 1) {
          for (let j = 0; j < 3; j++) {
            ul[j] = { color: 'black', type: 'normal' }
          }
          ul[1] = { color: '#0CCB8A', type: '800' }
          uc(ekc)
        } else if (i % 3 === 2) {
          for (let j = 0; j < 3; j++) {
            ul[j] = { color: 'black', type: 'normal' }
          }
          ul[2] = { color: '#0CCB8A', type: '800' }
          uc(erc)
        }
        i++
      }, 4000)
